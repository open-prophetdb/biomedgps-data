...
def get_pathway_lst(organism):
  ...
  for index, pathway in pathways.iterrows():
    ...
    filtered_descriptions = list(filter(lambda x: type(x) == dict and x.get('@Source', '') == 'WikiPathways-description', descriptions))
...
def extract(output):
  ...
  for organism in organisms.keys():
    ...
    pathway_df = pd.concat([pathway_df, pd.DataFrame(pathway_lst)], ignore_index=True)
