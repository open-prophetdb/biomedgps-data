...
def check_id(input_file, which_type):
    ...
    if which_type == "relation":
        for col in ["source_id", "target_id", "target_type", "source_id", "relation_type"]:
        ...
        for idx, row in df.iterrows():
            ...
            if not _check_relation_type(row["relation_type"]):
                show_msg(f"{row['relation_type']} is not a valid relation type at row {idx}. {row['source_type']} {row['target_type']}")
    ...
    if issues:
        for issue in issues:
            ...
            print(f"Found {len(issues)} issues in the input file {input_file}, please check it manually or remove --dry-run to correct it directly.")


@cli.command(help="Correct the relation file.")
@click.option("--uncorrected-file", "-r", help="The uncorrected relation file.", required=True)
@click.option("--relation-type-map", "-m", help="The relation type map file.")
@click.option("--which-type", "-w", help="The type of the relation to be corrected.", type=click.Choice(["relation", "relation_type_embedding"]), required=True)
def correct_relation(uncorrected_file, relation_type_map, which_type):
    ...
    if which_type == "relation_type_embedding":
        for col in [
            ...
            if col not in df.columns:
                raise Exception(f"{col} is missing in the relation_type_embedding file.")
    ...
    if which_type == "relation":
        ...
        if relation_type_map is not None:
            ...
            # Replace the relation type with the corrected relation type
            print("Replacing the relation type with the corrected relation type...")
            df.loc[df["relation_type"].isin(relation_type_df["relation_type"]), "relation_type"] = df.loc[
                df["relation_type"].isin(relation_type_df["relation_type"]), "relation_type"
            ].map(relation_type_df.set_index("relation_type")["corrected_relation_type"])

        print("The source and target type in the relation type are not consistent with the source and target type in the relation, correcting it...")
        for idx, row in df.iterrows():
            relation_type = row["relation_type"]
...
@cli.command(help="Check if all entities in the relation / entity embedding file are in the entity file.")
@click.option("--unchecked-file", "-r", help="A file is to be checked. It can be a relation file or an entity embedding file.", required=True)
...
def check_entity(unchecked_file, entity_file):
    ...
    node_ids = [f"{entity_type}::{entity_id}" for entity_type, entity_id in zip(entity_df["label"], entity_df["id"])]

    issues = []
    # It's a relation file
    if "source_id" in unchecked.columns and "target_id" in unchecked.columns:
        source_node_ids = list(set([f"{entity_type}::{entity_id}" for entity_type, entity_id in zip(unchecked["source_type"], unchecked["source_id"])]))
        for node_id in source_node_ids:
            if node_id not in node_ids:
                show_msg(f"Source {node_id} is not in the entity file.")

        target_node_ids = list(set([f"{entity_type}::{entity_id}" for entity_type, entity_id in zip(unchecked["target_type"], unchecked["target_id"])]))
    ...
    if "entity_id" in unchecked.columns and "entity_type" in unchecked.columns:
        for entity_type, entity_id in zip(unchecked["entity_type"], unchecked["entity_id"]):
