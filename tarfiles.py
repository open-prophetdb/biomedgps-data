...
def graph_data(destfile):
    ...
    graph_data_files = [os.path.join(current_dir, file) for file in graph_data_files]

    print("Creating tarball...")
    print("Checking if all files exist...")
    if not check_files(graph_data_files):
        return
    print("All files exist!")

    with open(md5file, "w") as f:
        for item in list_files_md5sum(graph_data_files):
            f.write(f"{item['md5sum']} {item['file']}\n")

    graph_data_files.append(md5file)
    create_tarball(graph_data_files, destfile)


@cli.command(help="Wrap all initial embedding files into a tarball")
@click.argument("destfile", type=click.Path(exists=False))
def initial_embedding(destfile):
    current_dir = os.path.abspath(os.path.dirname(__file__))
    ...
    print("Creating tarball...")
    print("Checking if all files exist...")
    if not check_files(initial_embedding_files):
        return
    print("All files exist!")

    with open(md5file, "w") as f:
        for item in list_files_md5sum(initial_embedding_files):
            f.write(f"{item['md5sum']} {item['file']}\n")

    initial_embedding_files.append(md5file)
    create_tarball(initial_embedding_files, destfile)
